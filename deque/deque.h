...
//
// 实现说明:
//     deque 使用二级指针结构（map + buffer）实现，其中 map 是指向 buffer
//     的指针数组， 每个 buffer
//     包含固定数量（m_buffer_size）的元素。这种设计使得 deque 能够在
//     两端高效扩展，同时保持良好的内存局部性。
//
// 许可证:
//     MIT License
...
/// @brief deque 容器类
/// @details
/// deque（双端队列）是一种高效的序列容器，支持在头尾两端快速插入和删除元素，
///          并提供随机访问能力。deque 采用分段存储结构，通过 map 数组管理多个
///          buffer， 每个 buffer 存储固定数量的元素。
...
class deque
{
    ...
    private:
        ...
        using map_alloc_traits = std::allocator_traits<map_allocator_type>;
...
inline deque<T, Allocator>::iterator::iterator()
    : m_cur(nullptr), m_first(nullptr), m_last(nullptr), m_map_node(nullptr)
{
}
