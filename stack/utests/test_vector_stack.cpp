/**
 * @file test_vector_stack.cpp
 * @brief 基于doctest的工业级栈(stack)测试套件 - 使用vector作为底层容器
 *
 * 测试覆盖范围:
 * - 构造函数(默认、容器、拷贝、移动)
 * - 赋值操作符(拷贝赋值、移动赋值)
...
TEST_SUITE("Vector Stack Constructors")
{
    TEST_CASE("Default constructor creates empty stack")
    {
        demo::stack<int, demo::vector<int>> stk;
        CHECK(stk.empty());
        CHECK(stk.size() == 0);
    }
    ...
    TEST_CASE("Constructor with container")
    {
        ...
        demo::stack<int, demo::vector<int>> stk(vec);
        CHECK(stk.size() == 5);
    ...
    {
        demo::stack<int, demo::vector<int>> stk1;
        stk1.push(1);
        stk1.push(2);
        stk1.push(3);

        demo::stack<int, demo::vector<int>> stk2(stk1);
        CHECK(stk2.size() == 3);
        CHECK(stk2.top() == 3);
    ...
    TEST_CASE("Move constructor")
    {
        ...
        demo::stack<int, demo::vector<int>> stk2(std::move(stk1));
...
TEST_SUITE("Vector Stack Assignment Operators")
{
    TEST_CASE("Copy assignment operator")
    {
        ...
        stk1.push(1);
        stk1.push(2);
        demo::stack<int, demo::vector<int>> stk2;
        stk2.push(10);
...
TEST_SUITE("Vector Stack Element Access")
{
    ...
    TEST_CASE("const top() returns const reference")
    {
        ...
        const demo::stack<int, demo::vector<int>> &const_stk = stk;
...
TEST_SUITE("Vector Stack Capacity")
{
...
TEST_SUITE("Vector Stack Modifiers")
{
    ...
    {
        demo::stack<std::string, demo::vector<std::string>> stk;
        stk.emplace("hello");
...
TEST_SUITE("Vector Stack Comparison Operators")
{
    TEST_CASE("operator== compares contents")
    {
        ...
        demo::stack<int, demo::vector<int>> stk3;
        stk3.push(1);
        stk3.push(3);
...
TEST_SUITE("Vector Stack Edge Cases")
{
...
TEST_SUITE("Vector Stack Performance")
{
