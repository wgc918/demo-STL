...
//
// 描述:
//     本文件实现了一个 STL 风格的 multimap 容器。
//     multimap 是一种关联容器，存储键值对（key-value pairs），
//     允许键重复，且按键的顺序进行排序。
//
// 功能特性:
...
//
// 实现说明:
//     multimap 使用红黑树（Red-Black Tree）数据结构实现，这是一种自平衡二叉搜索树。
//     红黑树通过维护节点颜色和旋转操作保持树的平衡，确保最坏情况下的时间复杂度。
//     与 map 不同，multimap 在插入时对重复键不做排斥，而是按等价键顺序插入。
//
// 许可证:
//     MIT License
//
...
/// @brief multimap 容器类
/// @details
/// multimap 是一种关联容器，存储键值对（key-value
/// pairs），允许键重复，且按键的顺序进行排序。 multimap 使用红黑树实现，保证 O(log
/// n) 的插入、删除和查找操作。
...
/// @param lhs 左操作数
/// @param rhs 右操作数
/// @return 如果两个 multimap 相等返回 true，否则返回 false
template <typename K, typename T, typename Compare, typename Allocator>
bool operator==(const multimap<K, T, Compare, Allocator>& lhs,
                const multimap<K, T, Compare, Allocator>& rhs);

/// @brief 比较两个 multimap 是否不相等
/// @tparam K 键类型
/// @tparam T 值类型
/// @tparam Compare 比较函数对象类型
/// @tparam Allocator 分配器类型
/// @param lhs 左操作数
/// @param rhs 右操作数
/// @return 如果两个 multimap 不相等返回 true，否则返回 false
template <typename K, typename T, typename Compare, typename Allocator>
bool operator!=(const multimap<K, T, Compare, Allocator>& lhs,
                const multimap<K, T, Compare, Allocator>& rhs);
...
public:
    /// @brief 已排序标记
    /// @details 用于从已排序的 vector 构造 multimap，可优化构建效率
    struct sorted_tag
    {
    };

    /// @brief 未排序标记
    /// @details 用于从未排序的 vector 构造 multimap
    struct unsorted_tag
    {
    };
...
public:
    ...
    /// @brief 双向迭代器类
    /// @details multimap 的迭代器，支持双向遍历（++ 和 --），按键的顺序访问元素
    class iterator
    {
    ...
    public:
        ...
        /// @param node 节点指针
        /// @param container multimap 容器指针
        iterator(Node* node, multimap* container);
    ...
    private:
        Node*     m_node;       ///< 当前节点指针
        multimap* m_container;  ///< 所属容器指针
    };

    /// @brief 常量双向迭代器类
    /// @details multimap 的常量迭代器，不允许修改指向的元素，支持双向遍历
    class const_iterator
    {
        friend class multimap;
    ...
    public:
        ...
        /// @brief 构造函数，从节点指针和容器指针创建迭代器
        /// @param node 节点指针
        /// @param container multimap 容器指针（const）
        const_iterator(Node* node, const multimap* container);
    ...
    private:
        Node*           m_node;       ///< 当前节点指针
        const multimap* m_container;  ///< 所属容器指针（const）
    };
...
public:
    ...
    /// @brief 默认构造函数，创建空 multimap
    multimap();

    /// @brief 构造函数，使用指定的比较函数创建空 multimap
    /// @param comp 比较函数对象
    ...
    explicit multimap(const std::vector<value_type>& vec, sorted_tag);

    /// @brief 非标准接口：从未排序的 vector 构造 multimap
    /// @param vec 未排序的键值对 vector
    /// @param tag 未排序标记（unsorted_tag）
    explicit multimap(const std::vector<value_type>& vec, unsorted_tag);

    /// @brief 拷贝构造函数
    /// @param other 要拷贝的 multimap
    multimap(const multimap& other);

    /// @brief 移动构造函数
    /// @param other 要移动的 multimap
    ...
    /// @brief 返回 multimap 中的元素数量
    /// @return 元素数量（包含重复键的元素）
    size_type size() const;

    /// @brief 检查 multimap 是否为空
    /// @return 如果为空返回 true，否则返回 false
    bool empty() const;

    /// @brief 返回 multimap 能容纳的最大元素数量
    /// @return 最大元素数量
    size_type max_size() const;

    // 修改器

    /// @brief 清空 multimap，删除所有元素
    void clear();

    /// @brief 插入元素（拷贝版本）
    /// @param value 要插入的键值对
    /// @return 指向插入元素的迭代器（multimap 总是插入成功）
    ...
    /// @brief 删除指定键的所有元素
    /// @param key 要删除的键
    /// @return 删除的元素数量（multimap 中可能删除多个）
    ...
    /// @brief 交换两个 multimap 的内容
    /// @param other 要交换的 multimap
    void swap(multimap& other);

    /// @brief 合并另一个 multimap 的元素
    /// @tparam Compare2 另一个 multimap 的比较函数类型
    /// @param other 要合并的 multimap，合并后 other 变为空
    template <typename Compare2>
    void merge(multimap<K, T, Compare2, Allocator>& other);

    // 查找

    /// @brief 统计指定键的元素数量
    /// @param key 要查找的键
    /// @return 指定键的元素数量（multimap 中可能大于 1）
...
private:
    ...
    /// @brief 递归拷贝节点
    /// @param cur 当前节点
    /// @param other 源 multimap
    /// @return 拷贝后的节点
    Node* copy_node(Node* cur, const multimap& other);
...
template <typename K, typename T, typename Compare, typename Allocator>
multimap<K, T, Compare, Allocator>::iterator::iterator(const iterator& other)
...
multimap<K, T, Compare, Allocator>::iterator::operator=(const iterator& other)
{
    m_node      = other.m_node;
...
template <typename K, typename T, typename Compare, typename Allocator>
inline typename multimap<K, T, Compare, Allocator>::iterator::pointer
multimap<K, T, Compare, Allocator>::iterator::operator->() const
{
    return &m_node->value;
}

template <typename K, typename T, typename Compare, typename Allocator>
inline typename multimap<K, T, Compare, Allocator>::iterator::reference
multimap<K, T, Compare, Allocator>::iterator::operator*() const
{
    return m_node->value;
}
...
template <typename K, typename T, typename Compare, typename Allocator>
inline typename multimap<K, T, Compare, Allocator>::iterator&
multimap<K, T, Compare, Allocator>::iterator::operator++()
{
...
template <typename K, typename T, typename Compare, typename Allocator>
inline typename multimap<K, T, Compare, Allocator>::iterator
multimap<K, T, Compare, Allocator>::iterator::operator++(int)
{
    iterator temp(*this);
    ++(*this);
    return temp;
...
template <typename K, typename T, typename Compare, typename Allocator>
inline typename multimap<K, T, Compare, Allocator>::const_iterator::pointer
multimap<K, T, Compare, Allocator>::const_iterator::operator->() const
{
    return &m_node->value;
}

template <typename K, typename T, typename Compare, typename Allocator>
inline typename multimap<K, T, Compare, Allocator>::const_iterator::reference
multimap<K, T, Compare, Allocator>::const_iterator::operator*() const
{
    return m_node->value;
}
...
template <typename K, typename T, typename Compare, typename Allocator>
inline typename multimap<K, T, Compare, Allocator>::const_iterator
multimap<K, T, Compare, Allocator>::const_iterator::operator++(int)
{
...
template <typename K, typename T, typename Compare, typename Allocator>
inline bool multimap<K, T, Compare, Allocator>::const_iterator::operator==(
...
template <typename K, typename T, typename Compare, typename Allocator>
multimap<K, T, Compare, Allocator>::multimap(const Compare& comp)
...
template <typename K, typename T, typename Compare, typename Allocator>
multimap<K, T, Compare, Allocator>::multimap(const multimap& other)
    : m_root(nullptr),
      m_nil(nullptr),
      m_size(other.m_size),
      m_comp(other.m_comp),
...
template <typename K, typename T, typename Compare, typename Allocator>
multimap<K, T, Compare, Allocator>::multimap(multimap&& other) noexcept
    : m_root(other.m_root),
      m_nil(other.m_nil),
      m_size(other.m_size),
...
template <typename K, typename T, typename Compare, typename Allocator>
inline typename multimap<K, T, Compare, Allocator>::allocator_type
multimap<K, T, Compare, Allocator>::get_allocator() const
{
    return typename multimap<K, T, Compare, Allocator>::allocator_type();
}

template <typename K, typename T, typename Compare, typename Allocator>
inline typename multimap<K, T, Compare, Allocator>::iterator
multimap<K, T, Compare, Allocator>::begin()
{
    Node* node = m_root;
    if (node != m_nil)
    {
...
template <typename K, typename T, typename Compare, typename Allocator>
inline typename multimap<K, T, Compare, Allocator>::const_iterator
multimap<K, T, Compare, Allocator>::begin() const
{
    Node* node = m_root;
    if (node != m_nil)
    {
...
template <typename K, typename T, typename Compare, typename Allocator>
inline typename multimap<K, T, Compare, Allocator>::const_iterator
multimap<K, T, Compare, Allocator>::cbegin() const noexcept
{
    return begin();
}
...
template <typename K, typename T, typename Compare, typename Allocator>
inline typename multimap<K, T, Compare, Allocator>::iterator
multimap<K, T, Compare, Allocator>::end()
{
...
template <typename K, typename T, typename Compare, typename Allocator>
inline typename multimap<K, T, Compare, Allocator>::const_iterator
multimap<K, T, Compare, Allocator>::end() const
{
...
template <typename K, typename T, typename Compare, typename Allocator>
inline typename multimap<K, T, Compare, Allocator>::const_iterator
multimap<K, T, Compare, Allocator>::cend() const noexcept
{
    return const_iterator(m_nil, this);
}

template <typename K, typename T, typename Compare, typename Allocator>
inline typename multimap<K, T, Compare, Allocator>::reverse_iterator
multimap<K, T, Compare, Allocator>::rbegin()
{
    return reverse_iterator(end());
}

template <typename K, typename T, typename Compare, typename Allocator>
inline typename multimap<K, T, Compare, Allocator>::const_reverse_iterator
multimap<K, T, Compare, Allocator>::rbegin() const
{
...
template <typename K, typename T, typename Compare, typename Allocator>
inline typename multimap<K, T, Compare, Allocator>::const_reverse_iterator
multimap<K, T, Compare, Allocator>::crbegin() const noexcept
{
    return const_reverse_iterator(cend());
}
...
template <typename K, typename T, typename Compare, typename Allocator>
inline typename multimap<K, T, Compare, Allocator>::reverse_iterator
multimap<K, T, Compare, Allocator>::rend()
{
...
template <typename K, typename T, typename Compare, typename Allocator>
inline typename multimap<K, T, Compare, Allocator>::const_reverse_iterator
multimap<K, T, Compare, Allocator>::rend() const
{
...
template <typename K, typename T, typename Compare, typename Allocator>
inline typename multimap<K, T, Compare, Allocator>::const_reverse_iterator
multimap<K, T, Compare, Allocator>::crend() const noexcept
{
    return const_reverse_iterator(cbegin());
}

template <typename K, typename T, typename Compare, typename Allocator>
inline typename multimap<K, T, Compare, Allocator>::size_type
multimap<K, T, Compare, Allocator>::size() const
{
    return m_size;
}

template <typename K, typename T, typename Compare, typename Allocator>
inline bool multimap<K, T, Compare, Allocator>::empty() const
{
    return m_root == m_nil;
}
...
template <typename K, typename T, typename Compare, typename Allocator>
inline typename multimap<K, T, Compare, Allocator>::size_type
multimap<K, T, Compare, Allocator>::max_size() const
{
    return alloc_traits::max_size(m_node_alloc);
}

template <typename K, typename T, typename Compare, typename Allocator>
inline void multimap<K, T, Compare, Allocator>::clear()
{
    if (empty())
        return;
...
template <typename K, typename T, typename Compare, typename Allocator>
inline typename multimap<K, T, Compare, Allocator>::iterator
multimap<K, T, Compare, Allocator>::insert(const value_type& value)
{
...
template <typename K, typename T, typename Compare, typename Allocator>
inline typename multimap<K, T, Compare, Allocator>::iterator
multimap<K, T, Compare, Allocator>::insert(value_type&& value)
{
    Node* parent = m_nil;
    Node* cur    = m_root;

    // multimap: 允许重复键，因此当键相等时，继续向右子树搜索
    // 这样新插入的重复键元素会被放置在已有相同键元素的右侧（保持插入顺序）
    while (cur != m_nil)
    {
        parent = cur;
    ...
    if (parent == m_nil)
        m_root = new_node;
    else if (m_comp(new_node->value.first, parent->value.first))
...
template <typename K, typename T, typename Compare, typename Allocator>
inline typename multimap<K, T, Compare, Allocator>::iterator
multimap<K, T, Compare, Allocator>::erase(const_iterator pos)
{
    Node* cur = pos.m_node;
    if (cur == m_nil)
        return iterator(m_nil, this);
...
template <typename K, typename T, typename Compare, typename Allocator>
inline typename multimap<K, T, Compare, Allocator>::size_type
multimap<K, T, Compare, Allocator>::erase(const key_type& key)
{
    // multimap: 删除所有匹配键的元素，返回删除的元素数量
    size_type count = 0;
...
template <typename K, typename T, typename Compare, typename Allocator>
inline void multimap<K, T, Compare, Allocator>::swap(multimap& other)
{
...
template <typename K, typename T, typename Compare, typename Allocator>
inline typename multimap<K, T, Compare, Allocator>::iterator
multimap<K, T, Compare, Allocator>::find(const key_type& key)
{
...
template <typename K, typename T, typename Compare, typename Allocator>
inline typename multimap<K, T, Compare, Allocator>::const_iterator
multimap<K, T, Compare, Allocator>::find(const key_type& key) const
{
...
template <typename K, typename T, typename Compare, typename Allocator>
inline void multimap<K, T, Compare, Allocator>::destroy(Node* node)
{
    if (node == m_nil)
        return;
...
template <typename K, typename T, typename Compare, typename Allocator>
inline void multimap<K, T, Compare, Allocator>::insert_balance(Node* node)
{
...
template <typename K, typename T, typename Compare, typename Allocator>
inline void multimap<K, T, Compare, Allocator>::erase_balance(Node* node)
{
...
template <typename K, typename T, typename Compare, typename Allocator>
inline typename multimap<K, T, Compare, Allocator>::Node*
multimap<K, T, Compare, Allocator>::rotate_left(Node* node)
{
...
template <typename K, typename T, typename Compare, typename Allocator>
inline typename multimap<K, T, Compare, Allocator>::Node*
multimap<K, T, Compare, Allocator>::rotate_right(Node* node)
{
    Node* parent          = node->parent;
...
template <typename K, typename T, typename Compare, typename Allocator>
inline typename multimap<K, T, Compare, Allocator>::Node*
multimap<K, T, Compare, Allocator>::find_node(const key_type& key)
{
    Node* cur = m_root;
    while (cur != m_nil)
    {
...
template <typename K, typename T, typename Compare, typename Allocator>
inline const typename multimap<K, T, Compare, Allocator>::Node*
multimap<K, T, Compare, Allocator>::find_node(const key_type& key) const
{
...
template <typename K, typename T, typename Compare, typename Allocator>
inline typename multimap<K, T, Compare, Allocator>::Node*
multimap<K, T, Compare, Allocator>::min_node(Node* node)
{
    while (node->left != m_nil)
        node = node->left;
    return node;
...
template <typename K, typename T, typename Compare, typename Allocator>
inline const typename multimap<K, T, Compare, Allocator>::Node*
multimap<K, T, Compare, Allocator>::min_node(const Node* node) const
{
    while (node->left != m_nil)
        node = node->left;
    return node;
...
template <typename K, typename T, typename Compare, typename Allocator>
inline typename multimap<K, T, Compare, Allocator>::Node*
multimap<K, T, Compare, Allocator>::max_node(Node* node)
{
...
template <typename K, typename T, typename Compare, typename Allocator>
inline const typename multimap<K, T, Compare, Allocator>::Node*
multimap<K, T, Compare, Allocator>::max_node(const Node* node) const
{
    while (node->right != m_nil)
        node = node->right;
    return node;
...
template <typename K, typename T, typename Compare, typename Allocator>
inline std::pair<typename multimap<K, T, Compare, Allocator>::iterator,
                 typename multimap<K, T, Compare, Allocator>::iterator>
multimap<K, T, Compare, Allocator>::equal_range(const key_type& key)
{
    return std::make_pair(lower_bound(key), upper_bound(key));
}

template <typename K, typename T, typename Compare, typename Allocator>
inline std::pair<typename multimap<K, T, Compare, Allocator>::const_iterator,
                 typename multimap<K, T, Compare, Allocator>::const_iterator>
multimap<K, T, Compare, Allocator>::equal_range(const key_type& key) const
{
    return std::make_pair(lower_bound(key), upper_bound(key));
}
...
template <typename K, typename T, typename Compare, typename Allocator>
inline typename multimap<K, T, Compare, Allocator>::iterator
multimap<K, T, Compare, Allocator>::lower_bound(const key_type& key)
{
    Node* cur    = m_root;
    Node* result = m_nil;
    while (cur != m_nil)
...
template <typename K, typename T, typename Compare, typename Allocator>
inline typename multimap<K, T, Compare, Allocator>::const_iterator
multimap<K, T, Compare, Allocator>::lower_bound(const key_type& key) const
{
    Node* cur    = m_root;
    Node* result = m_nil;
    while (cur != m_nil)
...
template <typename K, typename T, typename Compare, typename Allocator>
inline typename multimap<K, T, Compare, Allocator>::iterator
multimap<K, T, Compare, Allocator>::upper_bound(const key_type& key)
{
    Node* cur    = m_root;
    Node* result = m_nil;
    while (cur != m_nil)
...
template <typename K, typename T, typename Compare, typename Allocator>
inline typename multimap<K, T, Compare, Allocator>::const_iterator
multimap<K, T, Compare, Allocator>::upper_bound(const key_type& key) const
{
    Node* cur    = m_root;
    Node* result = m_nil;
    while (cur != m_nil)
...
template <typename K, typename T, typename Compare, typename Allocator>
inline typename multimap<K, T, Compare, Allocator>::key_compare_type
multimap<K, T, Compare, Allocator>::key_comp() const
{
    return m_comp;
}
...
#ifndef NDEBUG
template <typename K, typename T, typename Compare, typename Allocator>
inline bool multimap<K, T, Compare, Allocator>::validate_tree() const
{
    if (m_root == m_nil)
        return true;
...
template <typename K, typename T, typename Compare, typename Allocator>
inline bool operator==(const multimap<K, T, Compare, Allocator>& lhs,
                       const multimap<K, T, Compare, Allocator>& rhs)
{
