                   ...
                   const std::vector<std::pair<int, std::string>>& src, std::size_t n)
{
    ...
    }
    {
        ...
        double t = measure_median_us(op);
        writer.write_row({"map", "default_construct", n, "std", t});
    }
    ...
    }
    {
        ...
        double t = measure_median_us(op);
        writer.write_row({"map", "range_construct", n, "std", t});
    }
}
                ...
                const std::vector<std::pair<int, std::string>>& src, std::size_t n)
{
    ...
    }
    {
        ...
        double t = measure_median_us(op);
        writer.write_row({"map", "insert", n, "std", t});
    }
}
...
{
    {
        ...
        double t = measure_median_us(op);
        writer.write_row({"map", "find", n, "demo", t});
    }
    {
        ...
        double t = measure_median_us(op);
        writer.write_row({"map", "find", n, "std", t});
    }
}
...
{
    {
        ...
        double t = measure_median_us(op);
        writer.write_row({"map", "operator_at", n, "demo", t});
    }
    {
        std::map<int, std::string> std_prebuilt(prebuilt.begin(), prebuilt.end());
        auto                       op = [&]()
        ...
        double t = measure_median_us(op);
        writer.write_row({"map", "operator_at", n, "std", t});
    }
}
...
void run_erase(M& prebuilt, csv_writer::CsvWriter& writer, std::size_t n)
{
    std::size_t erase_count = n / 10 > 0 ? n / 10 : 1;
    {
        ...
        double t = measure_median_us(op);
        writer.write_row({"map", "erase", n, "demo", t});
    }
    {
        std::map<int, std::string> std_prebuilt(prebuilt.begin(), prebuilt.end());
        auto                       op = [&]()
        ...
        double t = measure_median_us(op);
        writer.write_row({"map", "erase", n, "std", t});
    }
}
...
{
    {
        ...
        double t = measure_median_us(op);
        writer.write_row({"map", "clear", n, "demo", t});
    }
    {
        std::map<int, std::string> std_prebuilt(prebuilt.begin(), prebuilt.end());
        auto                       op = [&]()
        ...
        double t = measure_median_us(op);
        writer.write_row({"map", "clear", n, "std", t});
    }
}
...
{
    {
        ...
        double t = measure_median_us(op);
        writer.write_row({"map", "forward_iterate", n, "demo", t});
    }
    {
        std::map<int, std::string> std_prebuilt(prebuilt.begin(), prebuilt.end());
        auto                       op = [&]()
        ...
        double t = measure_median_us(op);
        writer.write_row({"map", "forward_iterate", n, "std", t});
    }
}
...
void benchmark_map(csv_writer::CsvWriter& writer)
{
    ...
    for (std::size_t n : benchmark_config::DATA_SIZES)
    {
        ...
        // 预构建
        demo::map<int, std::string> demo_prebuilt;
        for (std::size_t i = 0; i < n; ++i)
