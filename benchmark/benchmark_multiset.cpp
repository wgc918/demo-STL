...
std::vector<int> generate_ints(std::size_t n)
{
    ...
    data.reserve(n);
    std::mt19937 rng(42);
    std::uniform_int_distribution<int> dist(1, n / 5 + 1); // 允许重复
...
{
    {
        ...
        writer.write_row({"multiset", "default_construct", n, "demo", t});
    }
    {
        auto op = [&]() {
            std::multiset<int> s;
        ...
        double t = measure_median_us(op);
        writer.write_row({"multiset", "default_construct", n, "std", t});
    }

    {
        ...
        double t = measure_median_us(op);
        writer.write_row({"multiset", "range_construct", n, "demo", t});
    }
    {
        auto op = [&]() {
            std::multiset<int> s(src.begin(), src.begin() + static_cast<std::ptrdiff_t>(n));
            sink = s.size();
        };
        double t = measure_median_us(op);
        writer.write_row({"multiset", "range_construct", n, "std", t});
    }
}
...
{
    {
        ...
        double t = measure_median_us(op);
        writer.write_row({"multiset", "insert", n, "demo", t});
    }
    {
        auto op = [&]() {
            std::multiset<int> s;
        ...
        double t = measure_median_us(op);
        writer.write_row({"multiset", "insert", n, "std", t});
    }
}
...
{
    {
        ...
        double t = measure_median_us(op);
        writer.write_row({"multiset", "find", n, "demo", t});
    }
    {
        std::multiset<int> std_prebuilt(prebuilt.begin(), prebuilt.end());
        auto op = [&]() {
            long long sum = 0;
            for (std::size_t i = 0; i < n; ++i)
            {
        ...
        double t = measure_median_us(op);
        writer.write_row({"multiset", "find", n, "std", t});
    }
}
...
void run_erase(S& prebuilt, csv_writer::CsvWriter& writer, std::size_t n)
{
    std::size_t erase_count = n / 10 > 0 ? n / 10 : 1;
    {
        ...
        double t = measure_median_us(op);
        writer.write_row({"multiset", "erase", n, "demo", t});
    }
    {
        std::multiset<int> std_prebuilt(prebuilt.begin(), prebuilt.end());
        auto op = [&]() {
            std::multiset<int> s(std_prebuilt);
            for (std::size_t i = 0; i < erase_count; ++i)
            {
                auto it = s.begin();
                if (it != s.end())
        ...
        double t = measure_median_us(op);
        writer.write_row({"multiset", "erase", n, "std", t});
    }
}
...
{
    {
        ...
        double t = measure_median_us(op);
        writer.write_row({"multiset", "clear", n, "demo", t});
    }
    {
        std::multiset<int> std_prebuilt(prebuilt.begin(), prebuilt.end());
        auto op = [&]() {
        ...
        double t = measure_median_us(op);
        writer.write_row({"multiset", "clear", n, "std", t});
    }
}
...
{
    {
        ...
        double t = measure_median_us(op);
        writer.write_row({"multiset", "forward_iterate", n, "demo", t});
    }
    {
        std::multiset<int> std_prebuilt(prebuilt.begin(), prebuilt.end());
        auto op = [&]() {
        ...
        double t = measure_median_us(op);
        writer.write_row({"multiset", "forward_iterate", n, "std", t});
    }
}
...
void benchmark_multiset(csv_writer::CsvWriter& writer)
{
    ...
    for (std::size_t n : benchmark_config::DATA_SIZES)
    {
        ...
        demo::multiset<int> demo_s;
        run_construct(demo_s, writer, src, n);

        demo::multiset<int> demo_prebuilt;
        for (std::size_t i = 0; i < n; ++i)
