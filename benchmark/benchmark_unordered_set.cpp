...
{
    {
        ...
        writer.write_row({"unordered_set", "default_construct", n, "demo", t});
    }
    {
        auto op = [&]()
        {
        ...
        double t = measure_median_us(op);
        writer.write_row({"unordered_set", "default_construct", n, "std", t});
    }

    {
        ...
        double t = measure_median_us(op);
        writer.write_row({"unordered_set", "range_construct", n, "demo", t});
    }
    {
        auto op = [&]()
        {
            std::unordered_set<int> s(src.begin(), src.begin() + static_cast<std::ptrdiff_t>(n));
            sink = s.size();
        };
        double t = measure_median_us(op);
        writer.write_row({"unordered_set", "range_construct", n, "std", t});
    }
}
...
{
    {
        ...
        double t = measure_median_us(op);
        writer.write_row({"unordered_set", "insert", n, "demo", t});
    }
    {
        auto op = [&]()
        {
        ...
        double t = measure_median_us(op);
        writer.write_row({"unordered_set", "insert", n, "std", t});
    }
}
...
{
    {
        ...
        double t = measure_median_us(op);
        writer.write_row({"unordered_set", "find", n, "demo", t});
    }
    {
        std::unordered_set<int> std_prebuilt(prebuilt.begin(), prebuilt.end());
        auto                    op = [&]()
        {
            long long sum = 0;
            for (std::size_t i = 0; i < n; ++i)
        ...
        double t = measure_median_us(op);
        writer.write_row({"unordered_set", "find", n, "std", t});
    }
}
...
void run_erase(S& prebuilt, csv_writer::CsvWriter& writer, std::size_t n)
{
    std::size_t erase_count = n / 10 > 0 ? n / 10 : 1;
    {
        ...
        double t = measure_median_us(op);
        writer.write_row({"unordered_set", "erase", n, "demo", t});
    }
    {
        std::unordered_set<int> std_prebuilt(prebuilt.begin(), prebuilt.end());
        auto                    op = [&]()
        {
            std::unordered_set<int> s(std_prebuilt);
            for (std::size_t i = 0; i < erase_count; ++i)
            {
                auto it = s.begin();
                if (it != s.end())
        ...
        double t = measure_median_us(op);
        writer.write_row({"unordered_set", "erase", n, "std", t});
    }
}
...
{
    {
        ...
        double t = measure_median_us(op);
        writer.write_row({"unordered_set", "clear", n, "demo", t});
    }
    {
        std::unordered_set<int> std_prebuilt(prebuilt.begin(), prebuilt.end());
        auto                    op = [&]()
        ...
        double t = measure_median_us(op);
        writer.write_row({"unordered_set", "clear", n, "std", t});
    }
}
...
{
    {
        ...
        double t = measure_median_us(op);
        writer.write_row({"unordered_set", "forward_iterate", n, "demo", t});
    }
    {
        std::unordered_set<int> std_prebuilt(prebuilt.begin(), prebuilt.end());
        auto                    op = [&]()
        ...
        double t = measure_median_us(op);
        writer.write_row({"unordered_set", "forward_iterate", n, "std", t});
    }
}
...
void benchmark_unordered_set(csv_writer::CsvWriter& writer)
{
    ...
    for (std::size_t n : benchmark_config::DATA_SIZES)
    {
        ...
        demo::unordered_set<int> demo_s;
        run_construct(demo_s, writer, src, n);

        demo::unordered_set<int> demo_prebuilt;
        for (std::size_t i = 0; i < n; ++i)
