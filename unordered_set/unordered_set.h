...
//
// 描述:
//     本文件实现了一个 STL 风格的 unordered_set 容器。
//     unordered_set 是一种关联容器，存储唯一的键（keys），
...
//
// 实现说明:
//     unordered_set 使用带链地址法（Separate Chaining）的哈希表实现。
...
#define UNORDERED_SET_DEFAULT_BUCKET_COUNT 16   ///< 默认桶数量
#define UNORDERED_SET_DEFAULT_LOAD_FACTOR  1.0  ///< 默认负载因子阈值

/// @brief unordered_set 容器类
/// @details
/// unordered_set 是一种关联容器，存储唯一的键（keys），
/// 键是唯一的，但不保证任何顺序。 unordered_set 使用哈希表实现，
...
/// @param lhs 左操作数
/// @param rhs 右操作数
/// @return 如果两个 unordered_set 相等返回 true，否则返回 false
/// @details
/// 两个 unordered_set 相等当且仅当它们具有相同数量的元素，
/// 且对于每个键 k 在 lhs 中，都存在一个键 k' 在 rhs 中使得 k == k'。
...
/// @brief 比较两个 unordered_set 是否不相等
/// @tparam Key 键类型
/// @tparam Hash 哈希函数对象类型
/// @tparam KeyEqual 键比较函数对象类型
/// @tparam Allocator 分配器类型
/// @param lhs 左操作数
/// @param rhs 右操作数
/// @return 如果两个 unordered_set 不相等返回 true，否则返回 false
template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
bool operator!=(const unordered_set<Key, Hash, KeyEqual, Allocator>& lhs,
                const unordered_set<Key, Hash, KeyEqual, Allocator>& rhs);

/// @brief unordered_set 类模板
/// @details
...
public:
    ...
    /// @brief 前向迭代器类
    /// @details
    /// unordered_set 的迭代器，支持前向遍历（++），按键的哈希值顺序访问元素。
    ...
    public:
        ...
        using value_type        = unordered_set::value_type;       ///< 元素类型
        using pointer           = unordered_set::pointer;          ///< 元素指针类型
        using reference         = unordered_set::reference;        ///< 元素引用类型
        using difference_type   = unordered_set::difference_type;  ///< 差值类型
    ...
    public:
        ...
        /// @param node 节点指针
        /// @param container unordered_set 容器指针
        iterator(Node* node, unordered_set* container);
    ...
    protected:
        Node*          m_node;       ///< 当前节点指针
        unordered_set* m_container;  ///< 所属容器指针
    };

    /// @brief 常量前向迭代器类
    /// @details
    /// unordered_set 的常量迭代器，不允许修改指向的元素，支持前向遍历。
    /// @note 迭代器在迭代过程中，如果容器被修改（插入或删除），迭代器可能失效。
    class const_iterator
    {
        friend class unordered_set;
    ...
    public:
        using iterator_category = std::forward_iterator_tag;       ///< 迭代器类别（前向迭代器）
        using value_type        = unordered_set::value_type;       ///< 元素类型
        using pointer           = unordered_set::const_pointer;    ///< 常量元素指针类型
        using reference         = unordered_set::const_reference;  ///< 常量元素引用类型
        using difference_type   = unordered_set::difference_type;  ///< 差值类型
    ...
    public:
        ...
        /// @brief 构造函数，从节点指针和容器指针创建迭代器
        /// @param node 节点指针
        /// @param container unordered_set 容器指针（const）
        const_iterator(Node* node, const unordered_set* container);
    ...
    protected:
        Node*                m_node;       ///< 当前节点指针
        const unordered_set* m_container;  ///< 所属容器指针（const）
    };

    /// @brief 桶内前向迭代器类
    /// @details
    /// unordered_set 的桶内迭代器，用于遍历单个桶中的所有元素。
    ...
    public:
        ...
        /// @param node 节点指针
        /// @param container unordered_set 容器指针（当前实现中未使用）
        /// @details
        /// 创建一个指向指定节点的桶内迭代器。
        /// container 参数仅为兼容性保留，桶内迭代器不跨桶移动。
        local_iterator(Node* node, unordered_set* container);
    ...
    /// @brief 桶内常量前向迭代器类
    /// @details
    /// unordered_set 的桶内常量迭代器，用于遍历单个桶中的所有元素，
    ...
    public:
        ...
        /// 创建一个指向指定节点的桶内常量迭代器。
        /// container 参数仅为兼容性保留，桶内迭代器不跨桶移动。
        const_local_iterator(Node* node, const unordered_set* container);
...
public:
    /// @brief 默认构造函数
    /// @details
    /// 创建一个空的 unordered_set，初始桶数量为
    /// UNORDERED_SET_DEFAULT_BUCKET_COUNT (16)。
    unordered_set();

    /// @brief 构造函数，指定初始桶数量
    /// @param bucket_count 初始桶数量
    /// @details
    /// 创建一个空的 unordered_set，使用指定的初始桶数量。
    ...
    /// UNORDERED_SET_DEFAULT_BUCKET_COUNT
    /// @details
    ...
    /// @param bucket_count 初始桶数量，默认为
    /// UNORDERED_SET_DEFAULT_BUCKET_COUNT
    /// @details 从初始化列表中的元素创建 unordered_set。
    unordered_set(std::initializer_list<value_type> ilist,
                  size_type bucket_count = UNORDERED_SET_DEFAULT_BUCKET_COUNT);

    /// @brief 拷贝构造函数
    /// @param other 要拷贝的 unordered_set
    /// @details 创建一个 unordered_set，作为 other 的副本。
    unordered_set(const unordered_set& other);

    /// @brief 移动构造函数
    /// @param other 要移动的 unordered_set
    /// @details 创建一个 unordered_set，移走 other 的内容。
    /// 移动后 other 处于有效但未定义的状态。
    unordered_set(unordered_set&& other) noexcept;

    /// @brief 拷贝赋值运算符
    /// @param other 要拷贝的 unordered_set
    /// @return 当前 unordered_set 的引用
    ...
    /// @brief 检查 unordered_set 是否为空
    /// @return 如果为空返回 true，否则返回 false
    bool empty() const noexcept;

    /// @brief 返回 unordered_set 中的元素数量
    /// @return 元素数量
    size_type size() const noexcept;

    /// @brief 返回 unordered_set 能容纳的最大元素数量
    /// @return 最大元素数量
    size_type max_size() const noexcept;

    // 修改器

    /// @brief 清空 unordered_set，删除所有元素
    ...
    ///         布尔值表示是否插入了新元素
    /// @details
    /// 使用 args 构造新的键并插入。如果键已存在，则不进行任何操作。
    ...
    /// @brief 合并异类型 unordered_set 的元素
    /// @tparam H2 另一个 set 的哈希函数类型
    /// @tparam P2 另一个 set 的键比较函数类型
    /// @param source 要合并的 unordered_set
    /// @details
    /// 将 source 中的所有元素移动到当前容器。
    /// source 的键类型必须与当前容器兼容。
    template <typename H2, typename P2>
    void merge(unordered_set<Key, H2, P2, Allocator>& source);

    /// @brief 合并异类型 unordered_set 的元素（右值版本）
    /// @tparam H2 另一个 set 的哈希函数类型
    /// @tparam P2 另一个 set 的键比较函数类型
    /// @param source 要合并的 unordered_set（右值）
    template <typename H2, typename P2>
    void merge(unordered_set<Key, H2, P2, Allocator>&& source);

    /// @brief 交换两个 unordered_set 的内容
    /// @param other 要交换的 unordered_set
    /// @details
    /// 交换当前容器和 other 的所有元素。
    /// 元素的键类型必须与当前容器兼容。
    ...
    /// @brief 统计指定键的元素数量（unordered_set 中键唯一，返回 0 或 1）
    /// @param key 要查找的键
    ...
    /// @brief 返回等于指定键的元素范围
    /// @param key 键
    /// @return 包含 lower_bound 和 upper_bound 的 pair（unordered_set
    /// 中范围最多包含一个元素）
...
//------------------------iterator 实现------------------------
template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
unordered_set<Key, Hash, KeyEqual, Allocator>::iterator::iterator()
...
template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
unordered_set<Key, Hash, KeyEqual, Allocator>::iterator::iterator(const iterator& other)
    : m_node(other.m_node), m_container(other.m_container)
{
}

template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
inline typename unordered_set<Key, Hash, KeyEqual, Allocator>::iterator::pointer
unordered_set<Key, Hash, KeyEqual, Allocator>::iterator::operator->() const
{
    return &m_node->value;
}

template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
inline typename unordered_set<Key, Hash, KeyEqual, Allocator>::iterator::reference
unordered_set<Key, Hash, KeyEqual, Allocator>::iterator::operator*() const
{
    return m_node->value;
}
...
template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
inline typename unordered_set<Key, Hash, KeyEqual, Allocator>::iterator&
...
template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
inline bool unordered_set<Key, Hash, KeyEqual, Allocator>::iterator::operator==(
...
template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
unordered_set<Key, Hash, KeyEqual, Allocator>::const_iterator::const_iterator(
    Node* node, const unordered_set* container)
    : m_node(node), m_container(container)
{
}
...
template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
inline typename unordered_set<Key, Hash, KeyEqual, Allocator>::const_iterator::pointer
unordered_set<Key, Hash, KeyEqual, Allocator>::const_iterator::operator->() const
{
    return &m_node->value;
}

template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
typename unordered_set<Key, Hash, KeyEqual, Allocator>::const_iterator::reference
unordered_set<Key, Hash, KeyEqual, Allocator>::const_iterator::operator*() const
{
    return m_node->value;
}
...
template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
inline typename unordered_set<Key, Hash, KeyEqual, Allocator>::const_iterator&
...
template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
inline bool unordered_set<Key, Hash, KeyEqual, Allocator>::const_iterator::operator==(
...
//------------------------local_iterator 实现------------------------
template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
unordered_set<Key, Hash, KeyEqual, Allocator>::local_iterator::local_iterator() : iterator()
{
}

template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
unordered_set<Key, Hash, KeyEqual, Allocator>::local_iterator::local_iterator(
    Node* node, unordered_set* container)
...
unordered_set<Key, Hash, KeyEqual, Allocator>::local_iterator::operator++()
{
...
template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
typename unordered_set<Key, Hash, KeyEqual, Allocator>::local_iterator
unordered_set<Key, Hash, KeyEqual, Allocator>::local_iterator::operator++(int)
{
...
template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
bool unordered_set<Key, Hash, KeyEqual, Allocator>::local_iterator::operator==(
...
template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
unordered_set<Key, Hash, KeyEqual, Allocator>::const_local_iterator::const_local_iterator()
    : const_iterator()
{
}
...
template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
inline typename unordered_set<Key, Hash, KeyEqual, Allocator>::const_local_iterator&
unordered_set<Key, Hash, KeyEqual, Allocator>::const_local_iterator::operator++()
{
    this->m_node = this->m_node->next;
    return *this;
}
...
template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
inline typename unordered_set<Key, Hash, KeyEqual, Allocator>::const_local_iterator
unordered_set<Key, Hash, KeyEqual, Allocator>::const_local_iterator::operator++(int)
{
...
template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
bool unordered_set<Key, Hash, KeyEqual, Allocator>::const_local_iterator::operator==(
...
unordered_set<Key, Hash, KeyEqual, Allocator>::unordered_set()
    : m_table(nullptr),
      m_mask(UNORDERED_SET_DEFAULT_BUCKET_COUNT - 1),
      m_bucket_count(UNORDERED_SET_DEFAULT_BUCKET_COUNT),
      m_size(0),
      m_max_load_factor(UNORDERED_SET_DEFAULT_LOAD_FACTOR),
      m_hash_function(),
      m_key_eq(),
      m_node_allocator(),
      m_bucket_allocator()
...
unordered_set<Key, Hash, KeyEqual, Allocator>::unordered_set(unordered_set&& other) noexcept
    : m_table(other.m_table),
      ...
      m_bucket_allocator(other.m_bucket_allocator)
{
    other.m_mask            = UNORDERED_SET_DEFAULT_BUCKET_COUNT - 1;
    other.m_bucket_count    = UNORDERED_SET_DEFAULT_BUCKET_COUNT;
    other.m_size            = 0;
    other.m_max_load_factor = UNORDERED_SET_DEFAULT_LOAD_FACTOR;
...
template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
inline unordered_set<Key, Hash, KeyEqual, Allocator>&
...
template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
inline bool unordered_set<Key, Hash, KeyEqual, Allocator>::empty() const noexcept
{
    return m_size == 0;
}

template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
inline typename unordered_set<Key, Hash, KeyEqual, Allocator>::size_type
unordered_set<Key, Hash, KeyEqual, Allocator>::size() const noexcept
{
    return m_size;
}
...
template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
inline void unordered_set<Key, Hash, KeyEqual, Allocator>::clear() noexcept
{
    destroy_all_nodes();
    m_size = 0;
}
...
template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
inline std::pair<typename unordered_set<Key, Hash, KeyEqual, Allocator>::iterator, bool>
...
template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
typename unordered_set<Key, Hash, KeyEqual, Allocator>::iterator
unordered_set<Key, Hash, KeyEqual, Allocator>::insert(const_iterator hint, value_type&& value)
{
    (void)hint;
...
template <typename... Args>
typename unordered_set<Key, Hash, KeyEqual, Allocator>::iterator
unordered_set<Key, Hash, KeyEqual, Allocator>::emplace_hint(const_iterator hint, Args&&... args)
{
...
template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
inline typename unordered_set<Key, Hash, KeyEqual, Allocator>::iterator
unordered_set<Key, Hash, KeyEqual, Allocator>::erase(const_iterator first, const_iterator last)
{
    while (first != last)
    {
        first = erase(first);
...
template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
inline typename unordered_set<Key, Hash, KeyEqual, Allocator>::size_type
unordered_set<Key, Hash, KeyEqual, Allocator>::erase(const Key& key)
{
...
template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
template <typename H2, typename P2>
inline void unordered_set<Key, Hash, KeyEqual, Allocator>::merge(
...
template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
void unordered_set<Key, Hash, KeyEqual, Allocator>::swap(unordered_set& other)
{
    using std::swap;
...
template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
inline typename unordered_set<Key, Hash, KeyEqual, Allocator>::size_type
unordered_set<Key, Hash, KeyEqual, Allocator>::count(const Key& key) const
{
...
template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
inline typename unordered_set<Key, Hash, KeyEqual, Allocator>::iterator
unordered_set<Key, Hash, KeyEqual, Allocator>::find(const Key& key)
{
...
template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
inline typename unordered_set<Key, Hash, KeyEqual, Allocator>::const_iterator
unordered_set<Key, Hash, KeyEqual, Allocator>::find(const Key& key) const
{
    Node* node = find_node(key);
...
template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
inline std::pair<typename unordered_set<Key, Hash, KeyEqual, Allocator>::iterator,
                 typename unordered_set<Key, Hash, KeyEqual, Allocator>::iterator>
unordered_set<Key, Hash, KeyEqual, Allocator>::equal_range(const Key& key)
{
...
template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
inline std::pair<typename unordered_set<Key, Hash, KeyEqual, Allocator>::const_iterator,
                 typename unordered_set<Key, Hash, KeyEqual, Allocator>::const_iterator>
unordered_set<Key, Hash, KeyEqual, Allocator>::equal_range(const Key& key) const
{
    size_type bucket_idx = bucket_index(key);
    Node*     head       = m_table[bucket_idx];
...
template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
inline typename unordered_set<Key, Hash, KeyEqual, Allocator>::local_iterator
unordered_set<Key, Hash, KeyEqual, Allocator>::begin(size_type n)
{
...
template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
inline typename unordered_set<Key, Hash, KeyEqual, Allocator>::const_local_iterator
unordered_set<Key, Hash, KeyEqual, Allocator>::begin(size_type n) const
{
...
template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
inline typename unordered_set<Key, Hash, KeyEqual, Allocator>::const_local_iterator
unordered_set<Key, Hash, KeyEqual, Allocator>::cbegin(size_type n) const
{
    return const_local_iterator(m_table[n], this);
}
...
template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
inline typename unordered_set<Key, Hash, KeyEqual, Allocator>::local_iterator
unordered_set<Key, Hash, KeyEqual, Allocator>::end(size_type n)
{
    (void)n;
...
template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
inline typename unordered_set<Key, Hash, KeyEqual, Allocator>::const_local_iterator
unordered_set<Key, Hash, KeyEqual, Allocator>::end(size_type n) const
{
    (void)n;
...
template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
inline typename unordered_set<Key, Hash, KeyEqual, Allocator>::const_local_iterator
unordered_set<Key, Hash, KeyEqual, Allocator>::cend(size_type n) const
{
    (void)n;
    return const_local_iterator(nullptr, this);
}
...
template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
inline typename unordered_set<Key, Hash, KeyEqual, Allocator>::size_type
unordered_set<Key, Hash, KeyEqual, Allocator>::max_bucket_count() const
{
...
template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
inline typename unordered_set<Key, Hash, KeyEqual, Allocator>::size_type
unordered_set<Key, Hash, KeyEqual, Allocator>::bucket_size(size_type n) const
{
    size_type size = 0;
    Node*     head = m_table[n];
    while (head != nullptr)
...
template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
inline typename unordered_set<Key, Hash, KeyEqual, Allocator>::size_type
unordered_set<Key, Hash, KeyEqual, Allocator>::bucket(const Key& k) const
{
    return bucket_index(k);
}

template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
inline float unordered_set<Key, Hash, KeyEqual, Allocator>::load_factor() const
{
    return static_cast<float>(size()) / m_bucket_count;
}

template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
inline float unordered_set<Key, Hash, KeyEqual, Allocator>::max_load_factor() const
{
    return m_max_load_factor;
}

template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
inline void unordered_set<Key, Hash, KeyEqual, Allocator>::max_load_factor(
...
template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
inline typename unordered_set<Key, Hash, KeyEqual, Allocator>::hash_type
...
template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
inline typename unordered_set<Key, Hash, KeyEqual, Allocator>::key_equal
...
unordered_set<Key, Hash, KeyEqual, Allocator>::next_power_of_two(size_type n) noexcept
{
    if (n == 0)
        return UNORDERED_SET_DEFAULT_BUCKET_COUNT;
...
template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
inline typename unordered_set<Key, Hash, KeyEqual, Allocator>::size_type
unordered_set<Key, Hash, KeyEqual, Allocator>::bucket_index(const Key& key) const
{
...
template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
inline typename unordered_set<Key, Hash, KeyEqual, Allocator>::size_type
unordered_set<Key, Hash, KeyEqual, Allocator>::bucket_index(uint64_t hash) const
{
    return hash & m_mask;
}

template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
inline typename unordered_set<Key, Hash, KeyEqual, Allocator>::Node*
unordered_set<Key, Hash, KeyEqual, Allocator>::find_node(const Key& key) const
{
...
template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
inline bool unordered_set<Key, Hash, KeyEqual, Allocator>::need_rehash() const
{
    return load_factor() > max_load_factor();
}

template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
void unordered_set<Key, Hash, KeyEqual, Allocator>::do_rehash(size_type new_bucket_count)
{
    if (new_bucket_count == m_bucket_count)
        return;

    if (new_bucket_count == 0)
        new_bucket_count = UNORDERED_SET_DEFAULT_BUCKET_COUNT;
...
template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
void unordered_set<Key, Hash, KeyEqual, Allocator>::destroy_all_nodes()
{
    for (size_type i = 0; i < m_bucket_count; i++)
    {
        Node* node = m_table[i];
